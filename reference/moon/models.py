...
from .utils import compute_accuracy, compute_adaptive_mu
...
def train_moon(
    ...
    use_bn_drift_control = (
        cfg is not None 
        and hasattr(cfg, 'bn_drift_control') 
        and cfg.bn_drift_control.enabled
    )
    
    # Get update frequency from config if available
    update_frequency = 100  # Default value
    if use_bn_drift_control and hasattr(cfg.bn_drift_control, 'update_frequency'):
        update_frequency = cfg.bn_drift_control.update_frequency
    
    # Store original mu value
    base_mu = mu
    
    # Compute initial adaptive mu based on BN spectrum similarity if enabled
    if use_bn_drift_control:
        adaptive_mu = compute_adaptive_mu(net, train_dataloader, device, base_mu, cfg)
        print(f"Round start - Base mu: {base_mu}, Adaptive mu: {adaptive_mu}")
    else:
        adaptive_mu = base_mu
    ...
    for epoch in range(epochs):
        ...
        for _, (x, target) in enumerate(train_dataloader):
            ...
            # Use adaptive mu instead of fixed mu
            loss2 = adaptive_mu * criterion(logits, labels)
            ...
            # Periodically recompute adaptive mu if enabled, always using original base_mu
            if use_bn_drift_control and cnt % update_frequency == 0:
                adaptive_mu = compute_adaptive_mu(net, train_dataloader, device, base_mu, cfg)
                print(f"Batch {cnt}, Adaptive mu: {adaptive_mu}")
